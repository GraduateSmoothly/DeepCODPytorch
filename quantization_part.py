...
def create_centers_variable(config):  # (C, L) or (L,)
    ...
    centers = torch.rand(config.num_centers, dtype=torch.float32) * (maxval - minval) - minval
    return centers
...
def _quantize1d(x, centers, sigma, data_format):
    ...
    ### symbols_hard 代表的是第几个center, (B,C,m,1)
    ...
    phi_hard = torch.zeros(symbols_hard.shape[0], num_centers).scatter_(1, symbols_hard, 1)
